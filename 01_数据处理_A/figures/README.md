...
- `fig_09_q1_method_flowchart.png`
  - 作用：展示第 1 问从原始数据、数据清洗、四类规律分析到 B/C 输出的建模思路。
  - 使用建议：用于论文第 1 问“问题分析”或“模型建立思路”小节，回应 C 侧关于建模过程引导不足的反馈。

## 同步规则

- 本目录中的 11 张图会同步复制到 `03_论文_C/图表汇总/`。
- 图表由 `04_代码/A_data_process.py` 自动生成，不建议手工覆盖。

## 复现方式

- 运行命令：`python 04_代码/A_data_process.py`
- 生成范围：阶段 1、阶段 2、阶段 3 全部结果，其中本目录会重生成 11 张图。
